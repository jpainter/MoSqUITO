...
ous_from_metatdata = function( .meta = NULL, 
    ...
    ous =  ous %>% 
        ...
        # MISTAKE: this one is suppose to remove leading zero from 09.33456, 
    # but it also removes zero from 9.0131
        # ous$str_js = gsub("(?<![0-9])0+", "", ous$str_js , perl = TRUE)
    
      # FIXED--nope
       # ous$str_js = gsub("0+(?<![0-9].)", "", ous$str_js , perl = TRUE)
    ...
    if ( SF ){
        ...
        ous.sf = left_join( ous.coord , ous , by = "orgUnit" )
        # glimpse(ous.df)
        
        # not run: plot polygons
        # plot(ous.sf %>% filter( feature %in% 'Polygon') %>% select( orgUnit.name, geometry))
    }
